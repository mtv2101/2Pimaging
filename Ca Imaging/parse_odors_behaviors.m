...
for b = 1:length(ALLBLOCKS)
    ...
    it2 = 1;
    trial = 1;
    for x = 1:size(ALLBLOCKS(b).dff,1)
        OdONFrameIndx = ALLBLOCKS(b).OdONFrameIndx;
        postwin = OdONFrameIndx(x,1)+postframes;
        prewin = OdONFrameIndx(x,1)-preframes;
        frmindx = [prewin:postwin];
        odorPres(x,:) = OdONFrameIndx(x,1) - prewin; %find odor presentation timepoint

        if o1(trial)
            o1_all(it1,:,:) = alldff(trial,frmindx,:);
            allblock_o1(it1,:,:,b) = o1_all(it1,:,:);
            it1=it1+1;
        elseif o2(trial)
            o2_all(it2,:,:) = alldff(trial,frmindx,:);
