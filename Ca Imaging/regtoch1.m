...
for n = 1:zdim
    ...
    tform = imregtform(signal(:,:,n),medref,'affine',optimizer,metric, 'PyramidLevels', 4);
    idx = 1:size(ref,3);
    %tform = sbxalign(double(ref),idx);
    signal_reg(:,:,n) = imwarp(signal(:,:,n), tform, 'linear', 'FillValues', 0, 'OutputView',imref2d(imagesize));
